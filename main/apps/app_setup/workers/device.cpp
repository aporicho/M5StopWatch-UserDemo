...
private:
    void createSwitchRow(int y, const char* title, bool initialValue, const std::function<void(bool)>& onChanged)
    {
        ...
        switch_widget->setBgColor(lv_color_hex(0x53BD65), checkedIndicator);
        switch_widget->setBgOpa(LV_OPA_COVER, checkedIndicator);
        switch_widget->setBorderWidth(0, checkedIndicator);
