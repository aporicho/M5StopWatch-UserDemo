...
export function telemetryRenderKey(envelope: TelemetryEnvelope | null) {
  ...
  return [
    ...
    telemetry.performance?.revision ?? 0,
    telemetry.performance?.current?.phase ?? "",
    telemetry.performance?.current?.elapsed_ms.toFixed(0) ?? "",
    telemetry.performance?.latest?.trace_id ?? "",
